...
- 后端支持直接通过 `python main.py` 启动（内部集成 `uvicorn main:app --reload` 逻辑）。
- 将 FastAPI 的 `@app.on_event` 启动/关闭事件改为官方推荐的 `lifespan` 写法，避免弃用告警。
- 调整“我的处理 / 我的审核”手机端顶部标签（未处理/已处理/待验收/未审核/已审核）的字体大小和间距：
  - 手机端：标签字体放大、上下间距增大、平分行宽并居中显示。
  - 电脑端：保持与 1.3 版本相同的紧凑布局。
